package aplicacion;

/**
 * Clase cereza la cual le brinda 10 puntos a mario(extiende objetoEstatico)
 * @author Cesar Gonzalez y Brayan Buitrago
 * @version 4/12/2019
*/

public class Cereza extends ObjetoEstatico {
	/**
	 * Constructor para la clase s
     * @param x posicion en coordenada x
     * @param y posicion en coordenada y
     * @param h alto del objeto
     * @param w ancho del objeto
	 */
    public Cereza(int x, int y, int h, int w)
    {
        super(x, y, h, w); 
    }
    
}
