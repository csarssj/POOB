package aplicacion;

/**
 * Clase Hongo  la cual mario sube para invertir las teclas(extiende objetoEstatico)
 * @author Cesar Gonzalez y Brayan Buitrago
 * @version 4/12/2019
*/

public class Hongo extends ObjetoEstatico {
	/**
	 * Constructor para la clase s
     * @param x posicion en coordenada x
     * @param y posicion en coordenada y
     * @param h alto del objeto
     * @param w ancho del objeto
	 */
    public Hongo(int x, int y, int h, int w)
    {
        super(x, y, h, w); 
    }
    
}
