...
public final class DonkeyPoob extends Observable implements Serializable {
    ...
    */
    ...
    public boolean startGame(PantallaJuego frame) throws InterruptedException, IOException
    {
        while(epochs < 10000) //despues de que epochs sea 10000, el juego terminara
        {
            ...
            for(int i = 0; i < MOList.size(); i++)
            {
                ...
                if(MOList.get(i) instanceof Barril) {
                	if (MOList.get(i).collidingWithMagic) {
                		MOList.remove(i);
    ...
    private void deleteMagic() {
    	for(int j = 0; j < SOList.size(); j++) {
			 if(SOList.get(j) instanceof Plataforma) {
    ...
    private void initStaticObjects() 
    {
		...
		if(random==0) {
			if(!magica) {
		        ...
		        {
		        	SOList.add(new PlataformaMagica(i,y,platform_HEIGHT,platform_WIDTH));
		            y = y - 1;
		        }
				magica = true;
			}else {
    ...
    {   
        ...
        //MOList.add(new BarrilRojo(120,200,20,25, SOList, true));
