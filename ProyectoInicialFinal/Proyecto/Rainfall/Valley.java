                ...
                public void addTarp(int[] ini,int[] fin)
                {
                        ...
                        nF[0]=fin[0];
                        nF[1]=maxY-fin[1];
                        if(interseccion(nI,nF)){
                            if(isVisible){
                                    JOptionPane.showMessageDialog(null,"La lona se cruza con otra");
                                }
                            ok=false;
                        }
                        else{
                            lonas.add(new Tarp(nI,nF));
                            if(isVisible){
                ...
                */
                ...
                public void addTarp(String type,int[] ini,int[] fin)
                {
                        ...
                        if(interseccion(nI,nF)){
                            ...
                            if (type.equals("hard")){
                                lonas.add(new Hard(nI,nF));
                            }else if (type.equals("flexible")){
                                lonas.add(new Flexible(nI,nF));
                            }else if (type.equals("crazy")){
                                lonas.add(new Crazy(nI,nF));
                            }else if (type.equals("radical")){
                                lonas.add(new Radical(nI,nF));
                            }else{
                                lonas.add(new Tarp(nI,nF));
                            }
                   ...
                   public void removeTarp(int position)
                   {
                        ...
                        else if(lonas.get(position-1).getTipo()=="hard"){
                                if (isVisible){
                                    JOptionPane.showMessageDialog(null,"No se puede eliminar lona hard");
                                }
                                ok=false;
                        }
                        else{
                               int[] pos=lonas.get(position-1).getPos()[0];
                               int[] pos2=lonas.get(position-1).getPos()[1]; 
                    ...
                    public void makePuncture(int Tarp, int x){
                        if(lonas.get(Tarp-1).getTipo()=="radical"){
                            ...
                            ultimaAccion = "removeTarp";
                            ultObj = new ArrayList();
                            ultObj.add(lonas.get(Tarp-1).getPos()[0]);
                            ultObj.add(lonas.get(Tarp-1).getPos()[1]);
                    ...
                    public String[] rainsfall(){
                        ...
                        String[] m = new String[mojados.size()];
                        int x = 0;
                    ...
                    public int[][][] rains(){
                        ...
                        for(int i =0; i<lluvias.size();i++){
                            int pos = lluvias.get(i).getGotas().size();
                            for(int x = 0; x<pos;x++){
                                rainInfo[i][x]=lluvias.get(i).getGotas().get(x).getPos();
                            }
                        }
                        return rainInfo;
                    }
                    ...
                    /**
                     * reinicia la lluvia si se agrega una lona o un hueco
                     */
                    private void restartLluvia(){
                            int[] puntosLluvia= new int[lluvias.size()];
                            Hashtable<String,Integer> puntosLluvias=new Hashtable<String,Integer>();
                            ArrayList<String> tipo = new ArrayList<String>();
                            int tam= lluvias.size();
                            for (int i=0;i<tam;i++){
                                
                                puntosLluvias.put(lluvias.get(i).getTipo(),lluvias.get(i).getX());
                                tipo.add(lluvias.get(i).getTipo());
                            }
                            for (int i=0; i<tam;i++){
                                stopRain(puntosLluvias.get(tipo.get(i)));
                                startRain(tipo.get(i),(int)puntosLluvias.get(tipo.get(i)));
                    
                            }
                    }
                    /**
                     * mentodo para ver si dos lonas se intersecan
                     * @param ini primer punto de la nueva lona
                     * @param fin segundo punto de la nueva lona
                     */
                    private boolean interseccion(int[] ini, int[] fin){
                        ArrayList<String> puntos= new ArrayList<String>();
                        int i=0;
                        Tarp prueba = new Tarp(ini,fin);
                        double p = prueba.getP();
                        double y = prueba.getY();
                        for (int x=ini[0];x<fin[0];x++){
                            double inter = p * x + y;
                            int[] punto={x,(int)inter+1};
                            puntos.add(Arrays.toString(punto));
                        }
                        for(Tarp t: lonas){
                            double p1 = t.getP();
                            double y1 = t.getY();
                            ArrayList<String> puntos1= new ArrayList<String>();
                            for (int x=t.getPos()[0][0];x<t.getPos()[1][0];x++){
                                double inter = p1 * x + y1;
                                int[] punto={x,(int)inter+1};
                                puntos1.add(Arrays.toString(punto));
                            }
                            for(int j=0; j<puntos.size();j++){
                                if (puntos1.contains(puntos.get(j))) {
                                 return true;
                                }
                            }
                        }
                    ...
                    /*private void ordenarTarp(){
                        ArrayList<Tarp> temp= new ArrayList<Tarp>();
                        for(Tarp t: lonas){
                            for (Tarp s: lonas){
                                if(t.getPos()[0][1]>=s.getPos()[0][1]){
                                    if(t.getPos()[0][1]==s.getPos()[0][1]){
                                        if(t.getPos()[1][1]>=s.getPos()[1][1]){
                                            temp.add(t);
                                        }
                                    }else{
                                        temp.add(t);
